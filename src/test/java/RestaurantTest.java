...
class RestaurantTest {
    ...
    //>>>>>>>>>>>>>>>>>>>>>>>>>>>ORDER<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    @Test
    public void get_total_order_value_should_return_sum_of_prices_given_in_menu_list(){
        assertThat(restaurant.getTotalOrderValue("Sweet corn soup", "Vegetable lasagne"),equalTo(Long.valueOf(388)));
    }
    //>>>>>>>>>>>>>>>>>>>>>>>>>>>ORDER<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
